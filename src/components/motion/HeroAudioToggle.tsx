...
export default function HeroAudioToggle() {
  ...
  useEffect(() => {
    ...
    return audio.subscribe(() => setMuted(audio.isMuted()));
  }, []);
  ...
  return (
    <button
      ...
      onClick={() => getHeroAudio().toggleMuted()}
      aria-pressed={!muted}
      aria-label={muted ? 'Unmute hero game sound' : 'Mute hero game sound'}
    >
      {muted ? (
      ...
      <span className="hero-audio-toggle__label">
        {muted ? 'Muted' : 'Sound'}
      </span>
    </button>
  );
